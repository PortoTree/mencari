...
const penggunaMatch = file.match(/<div className="flex items-center gap-3 pb-2">[\s\S]*?<h3 className="font-bold text-black dark:text-\[#E4E6EB\] text-\[15px\] leading-tight">Pengguna<\/h3>[\s\S]*?<\/div>\r?\n\s*<\/div>/);
if (penggunaMatch) {
  const orig = penggunaMatch[0];
  const newHTML = `<div className="flex items-center justify-between pb-2 relative">
