...
const startMarker = '{Array.from({ length: 11 }).map((_, i) => (';
const endMarker = '                  ))}';
...
if (startIndex !== -1 && endIndex !== -1) {
  const newCardHtml = `{Array.from({ length: 11 }).map((_, i) => (
                    <div key={i} className="bg-white dark:bg-[#242526] rounded-xl shadow-sm border border-gray-100 dark:border-[#3E4042] overflow-hidden hover:shadow-md transition-shadow cursor-pointer flex flex-col group">
                      ...
                      <div className="p-3 flex flex-col flex-1">
                        <div className="flex items-center gap-1.5 mb-2">
                          <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-gray-200 dark:bg-[#4E4F50] overflow-hidden shrink-0 flex items-center justify-center">
                             <img src="/default-avatar.svg" alt="Store" className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                          </div>
                          <span className="text-[11px] sm:text-[12px] font-medium text-gray-500 dark:text-[#B0B3B8] truncate">Toko Digital Kreatif {i + 1}</span>
                        </div>
...
`;
  ...
  // endIndex points to the start of "                  ))}"
  // So code.substring(endIndex) will naturally include "                  ))}"!
  // BUT wait, newCardHtml didn't have \n at the end. It's fine.
  
  code = code.substring(0, startIndex) + newCardHtml + code.substring(endIndex);
  fs.writeFileSync('src/app/[locale]/beranda/page.tsx', code);
  console.log('Successfully updated the dummy product card');
} else {
  console.log('Could not find the card block markers.');
}
