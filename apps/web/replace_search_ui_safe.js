...
if (startIdx !== -1) {
  const endStr = '                </div>';
  // Let's find the closing </div> of this block by finding the grid container which is right below it
  const gridStr = '<div className="grid grid-cols-2';
  const gridIdx = code.indexOf(gridStr, startIdx);
  
  if (gridIdx !== -1) {
    // Replace everything from startIdx up to gridIdx with our new code
    const blockToRemove = code.substring(startIdx, gridIdx);
    ...
    code = code.replace(blockToRemove, newUI);
