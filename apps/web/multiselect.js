...
// Replace layout
const layoutStartStr = '<div className="px-4 py-3 border-b border-gray-200 dark:border-[#3E4042] flex items-center gap-2">';
const layoutStartIdx = code.indexOf(layoutStartStr);

const layoutEndStr = '</div>\n              </div>\n             <div className="pt-4 px-4 border-b border-gray-200 dark:border-[#3E4042]">';
const layoutEndIdx = code.indexOf(layoutEndStr);

if (layoutStartIdx !== -1 && layoutEndIdx !== -1) {
  const newLayout = `<div className="px-4 py-3 border-b border-gray-200 dark:border-[#3E4042] flex items-center gap-2 flex-wrap min-h-[60px]">
  ...
  const endBlock = layoutEndIdx; 
  code = code.substring(0, layoutStartIdx) + newLayout + '\n              ' + code.substring(endBlock);
  fs.writeFileSync('src/app/[locale]/beranda/page.tsx', code);
  console.log('Replaced layout');
} else {
  console.log('Could not find layout bounds');
  console.log('Start index:', layoutStartIdx);
  console.log('End index:', layoutEndIdx);
}
