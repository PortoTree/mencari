...
// === Step 1: Add states after isChatFilterOpen ===
const stateInsertPoint = file.indexOf('const [isDarkMode, setIsDarkMode] = useState(true);');
if (stateInsertPoint !== -1) {
  const newStates = `const [activeChatMenu, setActiveChatMenu] = useState(null);
  const [menuPosition, setMenuPosition] = useState({ top: 0 });
  const chatMenuRef = React.useRef(null);
  `;
  file = file.slice(0, stateInsertPoint) + newStates + file.slice(stateInsertPoint);
  console.log('States added');
} else {
  console.log('ERROR: Could not find isDarkMode state');
}

// === Step 2: Add chatMenuRef check to outside click handler ===
// Find: setIsChatFilterOpen(false);\n      }\n    }\n    document.addEventListener
const filterClose = 'setIsChatFilterOpen(false);';
const pos = file.indexOf(filterClose);
if (pos !== -1) {
  const afterFilter = file.indexOf('}\n    document.addEventListener', pos);
  if (afterFilter !== -1) {
    const menuClose = `
      if (chatMenuRef.current && !chatMenuRef.current.contains(event.target)) {
        setActiveChatMenu(null);
      }`;
    file = file.slice(0, afterFilter + 1) + menuClose + '\n' + file.slice(afterFilter + 1);
    
    // Also change [] to [activeChatMenu] in the dependency array
    const depArrayPos = file.indexOf('}, []);', afterFilter);
    if (depArrayPos !== -1 && depArrayPos < afterFilter + 500) {
      file = file.slice(0, depArrayPos) + '}, [activeChatMenu]);' + file.slice(depArrayPos + 7);
      console.log('Outside click handler fixed');
    }
  } else {
    console.log('ERROR: Could not find closing bracket after filter');
  }
} else {
  console.log('ERROR: Could not find isChatFilterOpen close');
}

// === Step 3: Replace chat list item with hover menu version ===
...
const oldItemEnd = '</div>\n                  ))}';

const startPos = file.indexOf(oldItemStart);
const endPos = file.indexOf(oldItemEnd);

if (startPos !== -1 && endPos !== -1) {
  const newItem = `key={idx} className="relative group flex items-center gap-3 p-3 hover:bg-[#F2F2F2] dark:hover:bg-[#3A3B3C] cursor-pointer transition-colors">
  ...
  // Find the old item div content (from after key={idx}... to </div>\n))})
  const oldContent = file.slice(startPos, endPos + oldItemEnd.length);
  file = file.slice(0, startPos) + newItem + file.slice(endPos + oldItemEnd.length);
  console.log('Chat items updated');
} else {
  console.log('ERROR: Could not find chat item. startPos:', startPos, 'endPos:', endPos);
}

// === Step 4: Also add React import if useRef is used without React ===
// useRef is already imported, so chatMenuRef = React.useRef -> change to just useRef
file = file.replace('chatMenuRef = React.useRef(null);', 'chatMenuRef = React.createRef();');
// Actually use regular ref via useRef
file = file.replace('const chatMenuRef = React.createRef();', 'const chatMenuRef = useRef(null);');
