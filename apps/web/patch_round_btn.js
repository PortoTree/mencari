...
const newSwitcher = `{/* Language Switcher */}
          <div className="relative group" ref={langRef}>
            <button
              ...
              className="w-10 h-10 rounded-full bg-[#E4E6EB] dark:bg-[#3A3B3C] flex items-center justify-center text-black dark:text-[#E4E6EB] hover:bg-[#F3F2EF] dark:hover:bg-[#18191A] transition-colors overflow-hidden"
            >
              {locale === 'id' ? (
...
// Replace the old switcher block
...
if (startIdx !== -1 && endIdx !== -1) {
  ...
  console.log('✅ Replaced language switcher UI');
}
...
if (file.includes(handleOutsideStr) && !file.includes('langRef.current.contains')) {
  const replacement = `${handleOutsideStr}
      if (langRef.current && !langRef.current.contains(event.target as Node)) {
        setIsLangOpen(false);
      }`;
  file = file.replace(handleOutsideStr, replacement);
