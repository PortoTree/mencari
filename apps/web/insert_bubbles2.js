...
const bubblesToInsert = `
                ...
                <div className="flex items-end justify-end gap-2 max-w-[85%] self-end mt-2 ml-auto">
...
const startIdx = code.indexOf('{/* Floating Chat Room Panel */}');
if (startIdx !== -1) {
  const insertIdx = code.indexOf(anchor, startIdx);
  if (insertIdx !== -1) {
    const endOfAnchor = insertIdx + anchor.length;
    code = code.slice(0, endOfAnchor) + '\n' + bubblesToInsert + code.slice(endOfAnchor);
