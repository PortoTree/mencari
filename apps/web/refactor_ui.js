...
if (profileIdx !== -1) {
              ...
              <div className="w-full max-w-md bg-[#F0F2F5] dark:bg-[#18191A] rounded-2xl p-6 shadow-sm border border-gray-200 dark:border-[#3E4042] flex flex-col">
                 ...
                 <div className="pt-4 border-t border-gray-200 dark:border-[#3E4042] flex justify-end">
                    {selectedNewChatUsers.length === 1 ? (
                       <button onClick={() => { setActiveChatIdx(selectedNewChatUsers[0]); setSelectedNewChatUsers([]); setIsNewChatPanelOpen(false); }} className="bg-[#1877F2] hover:bg-blue-600 text-white font-semibold text-[15px] py-2.5 px-6 rounded-xl transition-colors flex items-center gap-2">
  ...
  // lines[profileIdx - 1] is expected to be `          ) : `
  // lines[profileIdx] is expected to be `          profileViewIdx !== null ? (`
  // Let's replace both with the new block
  lines.splice(profileIdx - 1, 2, newMiddleColumn);
} else {
  console.log('Middle column block not found.');
}
