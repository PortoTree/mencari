...
// 1. Fix the bubble padding issue by removing `w-full`
// Find the exact line in the Floating Chat Room Panel
const bubbleSearch = '<div className="flex flex-col gap-1 w-full">';
if (code.includes(bubbleSearch)) {
  // Let's do a more robust replace by finding it inside the floating chat room
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inFloatingRoom && lines[i].includes(bubbleSearch)) {
      lines[i] = lines[i].replace('w-full', '');
      break;
    }
  }
  code = lines.join('\n');
...
// 2. Change the Input Area of Floating Chat Room Panel
// We will locate the input area for Floating Chat Room and rewrite it.
const lines = code.split('\n');
let startInput = -1;
let endInput = -1;
let inFloatingRoom = false;
...
for (let i = 0; i < lines.length; i++) {
  ...
  // Let's find the end of Input area which is before {/* New Message Panel */}
  if (startInput !== -1 && lines[i].includes('{/* New Message Panel */}')) {
    // go back a few lines to keep the closing div of the panel
    endInput = i - 2; 
    break;
  }
}

if (startInput !== -1 && endInput !== -1) {
  ...
  lines.splice(startInput, endInput - startInput, ...newInput.split('\n'));
  fs.writeFileSync('src/app/[locale]/beranda/page.tsx', lines.join('\n'));
  console.log('Successfully updated input area and removed w-full from bubble');
}
