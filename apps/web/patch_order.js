...
const regex = /(<div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-\[\#242526\] rounded-lg shadow-\[0_0_15px_rgba\(0,0,0,0\.1\)\] border border-gray-100 dark:border-\[\#3E4042\] py-1\.5 z-50">)[\s\S]*?(<\/div>\s*\}\)\s*<\/div>)/;
...
if (match) {
  const newInside = `
                         ...
                         `;
  code = code.replace(regex, '$1' + newInside + '\n$2');
  fs.writeFileSync('src/app/[locale]/beranda/page.tsx', code);
  console.log('Successfully reordered dynamically');
} else {
