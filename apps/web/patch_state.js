...
// 1. Add imports for dynamic and Lottie and animationData
...
const newHomeTab = `<div onClick={() => setActiveTab('home')} className={\`flex flex-col items-center justify-center w-[110px] h-full border-b-[3px] cursor-pointer \${activeTab === 'home' ? 'border-emerald-500 text-emerald-500 dark:text-emerald-400 dark:border-emerald-400' : 'border-transparent text-gray-500 dark:text-[#B0B3B8] hover:bg-[#F2F2F2] dark:hover:bg-[#3A3B3C] rounded-lg my-1 transition-colors'}\`}>
            <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 20 20"><path d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-7z" /></svg>
            <span className="text-[11px] font-semibold mt-0.5">{t('tabs.home')}</span>
          </div>`;
...
const oldMencariTab = `<div onClick={() => router.push(\`/\${locale}/mencari\`)} className="flex flex-col items-center justify-center w-[110px] h-full text-gray-500 dark:text-[#B0B3B8] hover:bg-[#F2F2F2] dark:hover:bg-[#3A3B3C] rounded-lg cursor-pointer transition-colors my-1 border-b-[3px] border-transparent">
...
// 4. Wrap feed and add mencari view
const oldFeedStart = `<div className="w-full max-w-[680px] pt-6 pb-20">`;
const newFeedStart = `<div className="w-full max-w-[680px] pt-6 pb-20">
...
// Close the wrapper before the Profile Right Sidebar
const oldFeedEnd = `{/* Profile Right Sidebar */}`;
const newFeedEnd = `</>
          )}
...
console.log('✅ Updated beranda page with state');
