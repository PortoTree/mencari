...
// Header Mencari
const oldHeaderMencari = `<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>`;
const newHeaderMencari = `{activeTab === 'mencari' ? (
...
// Header Friends
const oldHeaderFriends = `<svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>`;
const newHeaderFriends = `{activeTab === 'friend' ? (
...
// Sidebar Mencari
const oldSidebarMencari = `<svg className="w-6 h-6 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>`;
const newSidebarMencari = `{activeTab === 'mencari' ? (
...
// Sidebar Friends
const oldSidebarFriends = `<svg className="w-6 h-6 text-black dark:text-[#E4E6EB]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>`;
const newSidebarFriends = `{activeTab === 'friend' ? (
...
file = file.split(oldHeaderMencari).join(newHeaderMencari);
file = file.split(oldHeaderFriends).join(newHeaderFriends);
file = file.split(oldSidebarMencari).join(newSidebarMencari);
file = file.split(oldSidebarFriends).join(newSidebarFriends);

fs.writeFileSync('src/app/[locale]/beranda/page.tsx', file);
console.log('✅ Applied conditional solid/outline SVGs');
