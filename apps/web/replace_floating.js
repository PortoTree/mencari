...
console.log('Floating Start:', start, 'End:', end);
if (start !== -1 && end !== -1) {
  const replacement = `              <div className="flex-1 flex items-center bg-[#F0F2F5] dark:bg-[#3A3B3C] border border-gray-300 dark:border-[#4E4F50] rounded-full px-3 py-1.5 min-w-0">
                <input 
                  type="text" 
              ...
              {floatingChatMessage.trim().length > 0 ? (
                <button className="bg-[#00B47A] text-white p-1.5 rounded-full hover:bg-[#009E6B] ml-1 transition-colors shrink-0 flex items-center justify-center w-8 h-8 shadow-sm">
