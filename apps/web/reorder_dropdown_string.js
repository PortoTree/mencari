...
const startTag = `<div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-[#242526] rounded-lg shadow-[0_0_15px_rgba(0,0,0,0.1)] border border-gray-100 dark:border-[#3E4042] py-1.5 z-50">`;
...
if (startIndex !== -1 && endIndex !== -1) {
  let block = code.substring(startIndex + startTag.length, endIndex);
  
  // We can see the parts in the block: manage chats, message requests, divider, settings
  // Let's just reconstruct the inside completely to be safe.
  const newInside = `
  ...
  code = code.substring(0, startIndex + startTag.length) + newInside + code.substring(endIndex);
